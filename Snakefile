...
rule clean:
    params: sge_opts=""
    shell: "rm -rf all_masked annotations_per_accession.tab bac* clones.* dotplot duplicated* error* fasta_index gaps* genes* gmap* liftover* mask mega64_out merged* mRNA* norptgap.tab post_mega rawFA ref* rmsk* segmental_duplications* tandem_repeats* wins wmsk* wssd* shared_sequence* gene_liftover* tracks blasr_alignments* clone_ids_summary.txt"
